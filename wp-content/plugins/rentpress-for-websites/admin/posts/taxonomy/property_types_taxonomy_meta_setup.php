...
<div class="rentpress-cpt-editor-container">
  ...
  <div class="rentpress-tabs">
    <div class="rentpress-tab-button" onclick="openTab(event, 'property_type-marketing')"><span class="fas fa-bullhorn"
        aria-hidden="true"></span> Marketing</div>
    <div class="rentpress-tab-button" onclick="openTab(event, 'property_type-info')"><span class="fas fa-info-circle"
  ...
  <div id="property_type-marketing" class="rentpress-tab-section">
    ...
    <div class="rentpress-panel">
      <p class="rentpress-panel-heading">Add an image to represent this property_type. For best results, use an image
        with landscape orientation and at least 1000px on the shortest side.</p>
    ...
    <div class="rentpress-panel">
      <p class="rentpress-panel-heading">Add a short summary description about this property_type. This description will
        also be used on property pages. For best results, keep less than 120 characters.</p>

      <div class="rentpress-settings-group">
        <label class="rentpress-settings-title">Short Description</label>
        <?php echo wp_kses(rentpress_metaShadowTextArea('rentpress_custom_field_property_type_short_description', $rpd, 6, '', 'description'), $rentpress_allowed_HTML); ?>
      </div>
    </div>
    <!-- tab accordion end -->
...
function rentpress_save_property_type_meta( $term_id ) {
    foreach ($_POST as $key => $value) {
        if ( strpos($key, 'rentpress_custom_field') !== false ) {
            ...
            if ($key === "rentpress_custom_field_property_type_extended_content" && isset($_POST[$key])) {

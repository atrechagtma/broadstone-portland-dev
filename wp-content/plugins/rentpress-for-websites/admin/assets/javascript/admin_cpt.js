...
function openTab(event, tabId) {
	...
	for (i = 0; i < tabButtons.length; i++) {
		tabButtons[i].className = tabButtons[i].className.replace(" rentpress-is-active", "");
	}
...
function toggleOverrides() {
	var overrideBoxes = this.parentElement.parentElement.parentElement.getElementsByClassName("rentpress-override");
...
function overrideFields() {
	var overrideCheckboxes = document.getElementsByClassName("rentpress-override");
	for (var i = 0; i < overrideCheckboxes.length; i++) {
	    ...
	    var inputName = overrideCheckboxName.substring(0, overrideCheckboxName.length-9);
...
function setUpOverrides(event) {
	var overrideCheckboxes = document.getElementsByClassName("rentpress-override");
	var overrideRangeCheckboxes = document.getElementsByClassName("rentpress-override-range");
	for (var i = 0; i < overrideCheckboxes.length; i++) {
...
function openEditUnitModal(unit) {
    var modalPadding = window.innerHeight*.1 + 'px ' + window.innerWidth*.1 + 'px';
    document.getElementById("rentpress-edit-unit-form-container").style.display = 'block';
    document.getElementById("rentpress-edit-unit-form-container").style.padding = modalPadding;
    document.body.style.overflow = 'hidden';

		var unitElements = document.getElementsByClassName('rentpress-edit-unit-form-value');
...
function closeEditUnitModal() {
    document.getElementById("rentpress-edit-unit-form-container").style.display = 'none';
...
function clearRequestResults() {
	setTimeout(function() {
		var requestResults = document.getElementsByClassName("rentpress-request-results");
...
function showDeleteConfirmation() {
	document.getElementById("rentpress-delete-confirmation-container").style.display = "block";
	document.getElementById("rentpress-edit-unit-button-container").style.display = "none";
}

function hideDeleteConfirmation() {
	document.getElementById("rentpress-edit-unit-button-container").style.display = "block";
	document.getElementById("rentpress-delete-confirmation-container").style.display = "none";
}

function filterPropertySelector() {
  var input = document.getElementById('rentpress-property-selector-search');
  var filter = input.value.toUpperCase();
  var properties = document.getElementsByClassName("rentpress-prop-selector-title");
...
jQuery(document).ready(function($) {
	...
	function refreshAddedUnits() {
	    var unitParentFloorplanCode = $("input[name='unit_parent_floorplan_code']").val();
	    var data = {
	...
	function addUnit() {
			...
			if ($('#rentpress-unit-code').hasClass('rentpress-unit-error') || $('#rentpress-unit-code').val() == '') {
				$("#results_for_new_unit").html("<span class='rentpress-error'>The Unit Code must be unique</span>");
				return;
			}
			// if unit name is empty, display error text and don't submit
			if ($('#rentpress-unit-name').val() == '') {
				$("#results_for_new_unit").html("<span class='rentpress-error'>The Unit Name must exist</span>");
				return;
			}
			// if unit date is empty, display error text and don't submit
			if ($('#rentpress-unit-ready-date').val() == '') {
				$("#results_for_new_unit").html("<span class='rentpress-error'>There must be an available date</span>");
				return;
			}

			jQuery.post(ajaxurl, data, function(response) {
				if (response == 0) {
						$("#results_for_new_unit").html("<span class='rentpress-error'>There was an error trying to add new unit</span>");
				} else {
						$("#results_for_new_unit").html("<span class='rentpress-success'>New unit successfully added</span>");
	...
	function editUnit() {
		...
		if ($('#rentpress-edit-unit-code').val() == '') {
			$("#results_for_edit_unit").html("<span class='rentpress-error'>The Unit Code must exist</span>");
			return;
		}
		// if unit name is empty, display error text and don't submit
		if ($('#rentpress-edit-unit-name').val() == '') {
			$("#results_for_edit_unit").html("<span class='rentpress-error'>The Unit Name must exist</span>");
			return;
		}
		// if unit date is empty, display error text and don't submit
		if ($('#rentpress-edit-unit-ready-date').val() == '') {
			$("#results_for_edit_unit").html("<span class='rentpress-error'>There must be an available date</span>");
			return;
		}

		jQuery.post(ajaxurl, data, function(response) {
			if (response == 0) {
				$("#results_for_edit_unit").html("<span class='rentpress-error'>There was an error trying to edit a unit</span>");
			} else {
				$("#results_for_edit_unit").html("<span class='rentpress-success'>Unit successfully edited</span>");
				$("input[name='unit_code']").val("");
				closeEditUnitModal();
				refreshAddedUnits();
			}
	...
	function deleteUnit() {
		var unitCode = $("#rentpress-edit-unit-code").value();
		var data = {
		...
		jQuery.post(ajaxurl, data, function(response) {
			if (response == 0) {
				$("#results_for_edit_unit").html("<span class='rentpress-error'>There was an error trying to delete unit</span>");
				clearRequestResults();
			} else {
				refreshAddedUnits();
				closeEditUnitModal();
...
jQuery(document).ready(function($){
	$( "#rentpress-marketing-resync-button" ).click(function() {
		...
		.done(function(response) {
				...
				if (!response.includes("Issue with connection") && !response.includes("error")) {
						$("#results_for_sync").html("<span style='color: #3399FF;'>Success! Your property data has been synced</span>");
				}
		})
		.fail(function(response) {
				console.log(response);
				if (response.status === 500) {
						$("#results_for_sync").html("It appears there is an issue communicating with the server, please try again later. If problem persists contact 30 Lines for assistance");
				}
		})
		.always(function() {
    ...
    // since 2.8 ajaxurl is always defined in the admin header and points to admin-ajax.php
    if (confirm("This action will add files to your currently active theme for your chosen posts and pages. Any customizations will be overwritten.")) {
        jQuery.post(ajaxurl, data, function(response) {
            console.log(response);
            if (response.includes("Success")) {
                $("#results_for_sync").html("<span style='color: #3399FF;'>Success! RentPress Templates have been added to your active theme.</span>");
            } else {
                $("#results_for_sync").html("Error! RentPress Templates could not be added to your theme. If this error persists, contact 30 lines for assistance.");
            }
        })
    }
});
	...
	var urlGet = url.searchParams.get("settingstab") ? url.searchParams.get("settingstab") : 'Feed Configuration';
	var urlTab = lastTab ? lastTab : urlGet;

	btn = document.getElementById('rentpress-marketing-resync-section');
	apiFields = document.querySelectorAll('#rentpress-api-credentials-settings-section input');
...
function makeOptionsPageConfig() {
	// this function is used to collect the diffrent fields and tabs to catigorize them for use in the tab switcher
	...
	if (tabsListEl) {
		...
		for (var i = sections.length - 1; i >= 0; i--) {
				...
				if (typeof section.dataset.page != 'undefined') {
						...
						tabListHtml.push(
								'<li id="rentpress-'+ page.replace(/\s/g , "-").toLowerCase() +'-option" class="rentpress-tabs-menu-item" onclick="changeSettingsTab(`'+ page +'`)"><h2><span>'+ page +'</a></h2></li>'
						);
						// make the tabs field array if it doesnt exist
...
function changeSettingsTab(tab) {
	...
	var refresh = window.location.protocol + "//" + window.location.host + window.location.pathname + "?page=rentpress&settingstab=" + tab;
	...
	activeTab = document.getElementById('rentpress-'+ tab.replace(/\s/g , "-").toLowerCase() +'-option');
	if (activeTab) {
...
function getParams(url) {
	// get the parms from a url string
	var params = {};
...
function getTemplateCheckBoxes() {
	return [
    	document.querySelector('input#rentpress_post_templates_property_archive_section'),
    	document.querySelector('input#rentpress_post_templates_property_single_section'),
    	document.querySelector('input#rentpress_post_templates_floorplan_archive_section'),
    	document.querySelector('input#rentpress_post_templates_floorplan_single_section'),
    	document.querySelector('input#rentpress_post_templates_amenity_single_section'),
    	document.querySelector('input#rentpress_post_templates_city_single_section'),
    	document.querySelector('input#rentpress_post_templates_city_archive_section'),
    	document.querySelector('input#rentpress_post_templates_property_taxonomy_single_section'),
    	document.querySelector('input#rentpress_post_templates_property_type_single_section'),
    	document.querySelector('input#rentpress_post_templates_pet_single_section'),
    	document.querySelector('input#rentpress_post_templates_feature_single_section'),
    ];
}
        ...
        click_elem.click(function(e) {
            ...
            if (limit == 'false') {
            	custom_uploader.on('select', function() {
	                ...
	                if (images.length) {
	                    ...
	                    for (var i = images.length - 1; i >= 0; i--) {
	                        image = images[i];
	                        imageUrl = typeof image.sizes.medium.url != undefined ? image.sizes.medium.url : image.url;
	                        imageHTML += "<div class='rentpress-gallery-single-image-wrapper'><span onclick='rentpressRemoveGalleryImage("+ image.id +", `"+ e.currentTarget.dataset.target +"`, this)'>X</span><img src='"+ imageUrl +"'></div>";
	                    }
	                    imageHTML += '</div>';
	                }
	                document.getElementById(e.currentTarget.dataset.target + '-upload-preview-container').innerHTML = imageHTML;
            	...
            	custom_uploader.on('select', function() {
	                var selection = custom_uploader.state().get('selection');
	                attachment = custom_uploader.state().get('selection').first().toJSON();
	                preview = document.getElementById(e.currentTarget.dataset.target + '-upload-preview-container');
	                image = attachment.url;

	                imageHTML = '';
	                if (image) {
	                	imageUrl = attachment.sizes.medium.url ? attachment.sizes.medium.url : attachment.url;
	                	imageHTML += '<img id="'+ e.currentTarget.dataset.target +'-image" class="rentpress-image-upload-preview" src="'+ imageUrl +'">';
	                }
    ...
    function rentpressClearPropertyGallery(event) {
	    ...
	    image_container = document.getElementById(parent.getElementsByClassName('rentpress-image-uploader-field')[0].dataset.target + '-upload-preview-container');
	    ...
	    for (var i = fields.length - 1; i >= 0; i--) {
	    	...
	    	field.addEventListener("change", function() {
	    		if (this.getAttribute('data-start-value') != this.value) {
	    			document.getElementById(this.getAttribute('data-base-field')).value = this.value;
	    		}
	    	});
	    }
